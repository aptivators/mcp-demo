"""Decorators for managing user permissions"""

import functools

try:
    from auth_groups_roles import check_user_permissions, get_user_admin_roles
    ...
    def wrapper(*args, **kwargs):
        if not AUTH_AVAILABLE:
            ...
            if kwargs.get("user_role") != "admin":
                return {
                    "error": "Admin access required. Authentication system not available."
                }
        ...
        try:
            ...
            return {"error": "Admin verification failed", "details": str(e)}
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if not AUTH_AVAILABLE:
                return {
                    "error": (
                        f"Group access required: {required_groups}. "
            ...
            try:
                ...
                permission_result = check_user_permissions(
                    required_groups=required_groups
                )
                ...
                if not permission_result.get("authorized", False):
                    return {
                        ...
                        "missing_groups": permission_result.get("groups", {}).get(
                            "missing_groups", []
                        ),
                        "user_groups": permission_result.get("groups", {}).get(
                            "user_groups", []
                        ),
                    }
                ...
                return {"error": "Group verification failed", "details": str(e)}
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            if not AUTH_AVAILABLE:
                return {
                    "error": (
                        f"Role access required: {required_roles}. "
                        "Authentication system not available."
                    )
                }
            ...
            try:
                ...
                if not permission_result.get("authorized", False):
                    return {
                        ...
                        "missing_roles": permission_result.get("roles", {}).get(
                            "missing_roles", []
                        ),
                        "user_roles": permission_result.get("roles", {}).get(
                            "user_roles", []
                        ),
                    }
                ...
                return {"error": "Role verification failed", "details": str(e)}
